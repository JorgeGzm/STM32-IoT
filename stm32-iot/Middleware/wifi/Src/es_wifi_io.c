...
typedef struct
{
	...
	GPIO_TypeDef* wifi_reset_gpio;
	uint16_t wifi_reset_pin;
	GPIO_TypeDef* wifi_csn_gpio;
	uint16_t wifi_csn_pin;
	GPIO_TypeDef* wifi_ready_gpio;
	uint16_t wifi_ready_pin;
}WifiDriverSPI_t;
...
static WifiDriverSPI_t wifiDriver;
...
void SPI_WIFI_attach(SPI_HandleTypeDef* xhspi,
		...
		uint16_t ready_pin)
{
	wifiDriver.wifi_reset_gpio = reset_gpio;
	wifiDriver.wifi_reset_pin = reset_pin;
	wifiDriver.spi = xhspi;
	wifiDriver.wifi_csn_gpio = csn_gpio;
	wifiDriver.wifi_csn_pin = csn_pin;
	wifiDriver.wifi_ready_gpio = ready_gpio;
	wifiDriver.wifi_ready_pin = ready_pin;
...
int8_t SPI_WIFI_ResetModule(void)
{
	...
	{
		Status = HAL_SPI_Receive(wifiDriver.spi, &Prompt[count], 1, 0xFFFF);
...
int16_t SPI_WIFI_ReceiveData(uint8_t *pData, uint16_t len, uint32_t timeout)
{
	...
	while (WIFI_IS_CMDDATA_READY())
	{
		if ((length < len) || (!len))
		{
			...
			if (HAL_SPI_Receive_IT(wifiDriver.spi, tmp, 1) != HAL_OK)
			{
				WIFI_DISABLE_NSS();
				UNLOCK_SPI();
...
int16_t SPI_WIFI_SendData(uint8_t *pdata, uint16_t len, uint32_t timeout)
{
	...
	if (len > 1)
	{
		...
		if (HAL_SPI_Transmit_IT(wifiDriver.spi, (uint8_t *) pdata, len / 2) != HAL_OK)
		{
			WIFI_DISABLE_NSS();
			UNLOCK_SPI();
			return ES_WIFI_ERROR_SPI_FAILED;
	...
	if (len & 1)
	{
		...
		if (HAL_SPI_Transmit_IT(wifiDriver.spi, Padding, 1) != HAL_OK)
		{
			WIFI_DISABLE_NSS();
			UNLOCK_SPI();
			return ES_WIFI_ERROR_SPI_FAILED;
...
{
	HAL_GPIO_WritePin(wifiDriver.wifi_reset_gpio, wifiDriver.wifi_reset_pin, GPIO_PIN_RESET);
	HAL_Delay(10);
...
{
	HAL_GPIO_WritePin( wifiDriver.wifi_csn_gpio, wifiDriver.wifi_csn_pin, GPIO_PIN_RESET );
}

void WIFI_DISABLE_NSS(void)
{
	HAL_GPIO_WritePin( wifiDriver.wifi_csn_gpio, wifiDriver.wifi_csn_pin, GPIO_PIN_SET );
}

uint8_t WIFI_IS_CMDDATA_READY(void)
{
	return (HAL_GPIO_ReadPin(wifiDriver.wifi_ready_gpio, wifiDriver.wifi_ready_pin) == GPIO_PIN_SET);
}
